...
pub struct WorkflowData {
  ...
  /// A semantic version to use when producing the artifacts. Must be in three parts separated by
  /// dot
...
impl AnagolayStructureData for WorkflowData {
  fn validate(&self) -> Result<(), Characters> {
    ...
    } else if self.version.split(".").len() != 3 {
      Err("WorkflowData.version: must be a semantic version with three parts separated by dot".into())
    } else {
      Ok(())
    }
  }
