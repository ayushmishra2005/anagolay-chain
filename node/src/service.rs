...
// Local Runtime Types
use anagolay_runtime::{opaque::Block, Hash, RuntimeApi};
...
impl sc_executor::NativeExecutionDispatch for ParachainNativeExecutor {
  ...
  fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
    anagolay_runtime::api::dispatch(method, data)
  }
    ...
    create_inherent_data_providers: move |_, (relay_parent, validation_data)| {
      ...
      async move {
        ...
        let parachain_inherent = parachain_inherent
          .ok_or_else(|| Box::<dyn std::error::Error + Send + Sync>::from("Failed to create parachain inherent"))?;
