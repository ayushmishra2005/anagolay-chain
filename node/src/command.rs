...
impl SubstrateCli for Cli {
  ...
  fn impl_version() -> String {
    format!(
      "{}-spec:{}",
      env!("IDIYANALE_CLI_IMPL_VERSION"),
      anagolay_runtime::VERSION.spec_version.to_string()
    )
  }
...
pub fn run() -> Result<()> {
  ...
  match &cli.subcommand {
    ...
    Some(Subcommand::Revert(cmd)) => {
      construct_async_run!(|components, cli, cmd, config| Ok(cmd.run(components.client, components.backend, None)))
    }
    ...
    Some(Subcommand::Benchmark(cmd)) => {
      ...
      match cmd {
        ...
        BenchmarkCmd::Machine(cmd) => runner.sync_run(|config| cmd.run(&config, SUBSTRATE_REFERENCE_HARDWARE.clone())),
    ...
    Some(Subcommand::TryRuntime(cmd)) => {
      if cfg!(feature = "try-runtime") {
        ...
        runner.async_run(|config| Ok((cmd.run::<Block, ParachainNativeExecutor>(config), task_manager)))
      } else {
